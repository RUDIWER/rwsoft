<template>
    <Head :title="t('categories.page_title', 'CMS categorieen')" />

    <AdminLayout :title="t('categories.page_title', 'CMS categorieen')">
        <Card class="rounded-none">
            ...
            >
                <div>
                    <CardTitle>{{
                        t('categories.title', 'Categorieen')
                    }}</CardTitle>
                    <CardDescription>
                        {{
                            t(
                                'categories.description',
                                'Beheer hierarchische berichtcategorieen.',
                            )
                        }}
                    </CardDescription>
                </div>
                <Button as-child>
                    <Link :href="route('admin.cms.categories.create')">
                        {{ t('categories.new', 'Nieuwe categorie') }}
                    </Link>
                </Button>
            </CardHeader>
            <CardContent class="p-0">
                <RwTable
                    table-id="admin-cms-categories-table"
                    :data="tableData"
                    :columns="columns"
...
const tableRows = computed(() =>
    props.categories.map((category) => ({
        ...
        active_label: category.is_active
            ? t('common.yes', 'Ja')
            : t('common.no', 'Nee'),
        updated_at_display: formatDate(category.updated_at),
    })),
);
...
const columns = computed(() => [
    ...
    {
        key: 'parent_title',
        label: t('common.columns.parent', 'Bovenliggend'),
        type: 'text',
        selected: true,
        sortable: true,
        filterable: true,
