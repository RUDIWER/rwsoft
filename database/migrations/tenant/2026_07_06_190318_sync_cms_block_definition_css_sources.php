...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach (app(CmsBlockRegistry::class)->all() as $blockKey => $definition) {
    ...
    {
        // Development data sync only. Do not remove block definitions or revisions from tenants.
    }

    private function hasRequiredTables(): bool
    {
        foreach (['cms_placeable_blocks', 'cms_placeable_block_revisions', 'cms_blocks'] as $table) {
    ...
    private function syncBlockDefinition(string $blockKey, array $definition): void
    {
        ...
        $payload = [
            ...
            'css_source' => $this->nullableString($definition['css_source'] ?? null),
    ...
    private function publishRevision(int $blockId, string $blockKey, array $payload, mixed $now): void
    {
        ...
        $previousLatestRevisionId = (int) DB::connection($this->connection)
        ...
        $snapshotHash = $this->snapshotHash($blockKey, $payload);
        $revisionId = (int) DB::connection($this->connection)
            ->table('cms_placeable_block_revisions')
            ->where('cms_placeable_block_id', $blockId)
        ...
        if ($revisionId <= 0) {
            ...
            $revisionId = (int) DB::connection($this->connection)
                ...
                ->insertGetId([
                    ...
                    'metadata' => json_encode(['source' => 'block_definition_css_sync'], JSON_THROW_ON_ERROR),
                    'published_at' => $now,
                    'created_at' => $now,
                    'updated_at' => $now,
                ]);
        ...
        $query = DB::connection($this->connection)
            ->table('cms_blocks')
            ->where('cms_placeable_block_id', $blockId)
            ->where(function ($query) use ($previousLatestRevisionId): void {
                $query->whereNull('placeable_block_revision_id');

                if ($previousLatestRevisionId > 0) {
                    $query->orWhere('placeable_block_revision_id', $previousLatestRevisionId);
                }
            });
     ...
     * @param  array<string, mixed>  $payload
     */
    private function snapshotHash(string $blockKey, array $payload): string
    {
    ...
    private function category(array $definition): string
    {
        ...
        return in_array($category, ['content', 'header', 'navigation', 'system', 'code', 'mail'], true)
            ? $category
            : 'content';
    }
