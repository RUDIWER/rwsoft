...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach ($this->templates() as $templateKey => $definition) {
            ...
            if ($this->templateHasActiveBuilderSections((int) $template->id)) {
                continue;
            }

            $this->createBuilderSection((int) $template->id, $templateKey, $definition);
        }
    }
    ...
    {
        // Backfill migration: existing templates may now depend on these sections.
    }

    private function hasRequiredTables(): bool
    {
        foreach (['cms_mail_templates', 'cms_placeable_blocks', 'cms_placeable_block_revisions', 'cms_sections', 'cms_blocks', 'cms_block_placements'] as $table) {
     ...
     */
    private function createBuilderSection(int $templateId, string $templateKey, array $definition): void
    {
        $now = now();
        ...
        DB::connection($this->connection)->table('cms_sections')->updateOrInsert(
            ...
            [
                ...
                'settings' => json_encode([
                    'source' => 'mail_template_builder_backfill',
    ...
    private function blockPayload(array $placeable, array $placement, mixed $now): array
    {
        $payload = [
            ...
            'settings' => json_encode(['source' => 'mail_template_builder_backfill'], JSON_THROW_ON_ERROR),
            'is_shared' => false,
            'is_dynamic' => false,
